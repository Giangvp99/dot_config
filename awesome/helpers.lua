...
helpers.start_monitor = function(script, callbacks)
	...
	local kill_monitor_script = string.format([[ pkill %s ]], helpers.get_main_process_name(script))
...
helpers.change_brightness = function(change_by)
	local cmd = change_by < 0 and "light -U " .. -change_by or "light -A " .. change_by
...
-- Language
-- ========================================
Languages = require("config.basics").Languages
