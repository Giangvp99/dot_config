...
local is_battery_low = function(percentage, status)
	return percentage >= 0 and percentage < low_battery_threshold and status ~= "Charging"
end
...
awesome.connect_signal("daemon::battery::status", function(_, stat)
	...
	if is_battery_low(stat.percentage, stat.status) and seconds_since_last_check > low_battery_reminder_interval then
