...
local Network_Interfaces = require("config.basics").Network_Interfaces
	...
	awful.spawn.easy_async_with_shell(wireless_data_script, function(data_stdout)
		awful.spawn.easy_async_with_shell(wireless_strength_script, function(strength_stdout)
			awful.spawn.easy_async_with_shell(healthcheck_script, function(health_stdout)
				...
				awesome.emit_signal("daemon::network::status::wireless", interface, healthy, essid, bitrate, strength)
			end)
		end)
	end)
end
...
local emit_wired_status = function()
	awful.spawn.easy_async_with_shell(healthcheck_script, function(stdout)
		...
		awesome.emit_signal("daemon::network::status::wired", interface, healthy)
	end)
end
...
local emit_wireless_connected = function()
	awful.spawn.easy_async_with_shell(wireless_data_script, function(data_stdout)
		...
		awesome.emit_signal("daemon::network::connected::wireless", interface, essid)
	end)
end
...
local emit_wired_connected = function()
	...
	awesome.emit_signal("daemon::network::connected::wired", interface)
end
...
local emit_disconnected = function()
	...
	awesome.emit_signal("daemon::network::disconnected::" .. network_mode, interface)
end
