...
local parse_stdout = function(stdout)
	...
	for s in stdout:gmatch("[^\r\n]+") do
		local status, charge_str, time = string.match(s, ".+: (%a+), (%d+)%%, (%d+:%d+:%d+) .*")
		if status ~= nil then
			table.insert(battery_info, { status = status, percentage = tonumber(charge_str), time = time })
		else
...
awful.spawn.easy_async_with_shell(charger_check_script, function(stdout, _, _, exit_code)
	...
	helpers.start_monitor(charger_monitor_script, { stdout = emit_charger_status(stdout) })
end)
