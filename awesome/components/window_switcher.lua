...
function Switcher:init_signals()
	...
	awesome.connect_signal("window_switcher::hide", function()
		self:hide()
	end)
end
...
function Switcher:get_mousebindings()
	return gears.table.join(
		...
		awful.button({ "Any" }, keys.leftclick, nil, function(c)
		...
		awful.button({ "Any" }, keys.midclick, nil, function(c)
			c:kill()
		end),

		-- Right click minimizes the client
		awful.button({ "Any" }, keys.rightclick, nil, function(c)
			c.minimized = true
		end),

		-- Scroll to switch focused client
		awful.button({ "Any" }, keys.scrolldown, client_commands.focus_next),
		awful.button({ "Any" }, keys.scrollup, client_commands.focus_prev),

		-- Side button up - toggle floating
		awful.button({ "Any" }, keys.sideupclick, function(c)
			c.floating = not c.floating
		end),

		-- Side button down - toggle ontop
		awful.button({ "Any" }, keys.sidedownclick, function(c)
			c.ontop = not c.ontop
		end)
	)
end
...
function Switcher:show()
	...
	if #self.screen.selected_tag:clients() == 0 then
		return
	end
...
function Switcher:hide()
	if client.focus then
		...
		if self.first_client then
			self.first_client:raise()
		end
		if self.last_client then
			self.last_client:raise()
		end
	end
